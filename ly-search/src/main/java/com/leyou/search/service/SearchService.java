...
public class SearchService {
    ...
    private List<Map<String, Object>> getSpecifications(Long id, QueryBuilder basicQuery) {
        // 1、根据分类查询规格
        ResponseEntity<String> specResp = this.specificationClient.querySpecById(id);
        if (!specResp.hasBody()) {
    ...
    private List<Brand> getBrandAgg(AggregatedPage<Goods> aggResult, String brandAggName) {
        ...
        ResponseEntity<List<Brand>> resp = this.brandsClient.queryBrandByIds(ids);
        if (resp.hasBody()) {
            return resp.getBody();
        }
        return null;
    ...
    private List<Category> getCategoryAgg(AggregatedPage<Goods> aggResult, String categoryAggName) {
        ...
        // 获取分类名称
        ResponseEntity<List<String>> resp = this.categoryClient.queryNameByIds(ids);
        if (!resp.hasBody()) {
            return null;
        }
    ...
    public void insertGoods(Long id){
